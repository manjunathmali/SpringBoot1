...
public class CarStoreService {
	...
	public List<Car> getCarSearchResult(CarSearch carSearch) throws ApplicationException {
		//Add code here..
		String brand = carSearch.getBrand();
		String fuelType = carSearch.getFuelType();
		String budget = carSearch.getBudgetUpto();
		double price =0.0;
		List<Car> list = new ArrayList<>();
		List<Car> cars = buildCars();
		if(budget.equalsIgnoreCase("3 lakh")) {
			price =3.0;
		}
		else if(budget.equalsIgnoreCase("5 lakh")) {
			price =5.0;
		}
		else if(budget.equalsIgnoreCase("7 lakh")) {
			price =7.0;
		}
		else if(budget.equalsIgnoreCase("10 lakh")) {
			price =10.0;
		}
